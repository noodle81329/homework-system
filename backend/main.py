...
def get_student_status(current_user: dict = Depends(auth.get_current_user)):
    ...
    folder_id = settings.get_target_folder_id()
    if not folder_id:
        return {
            "is_submitted": False,
            "submitted_at": None,
...
async def upload_homework(
    ...
    target_folder_id = settings.get_target_folder_id()
...
def get_all_submissions(current_user: dict = Depends(auth.get_current_user)):
